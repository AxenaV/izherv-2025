...
public class Character2DMovement : MonoBehaviour
{
    ...
    void AnimateCharacter()
    {
		...
		if (animator != null)
		{
			...
			// Orient the character: flip model to face movement direction.
			// Prefer flipping the selected character model if available.
			var modelGO = mSelector.character != null ? mSelector.character : gameObject;
			if (mInput.move.x > 0.0f && !mHeadingRight)
			{
				mHeadingRight = true;
				var s = modelGO.transform.localScale;
				s.x = Math.Abs(s.x);
				modelGO.transform.localScale = s;
			}
			...
			// Pass parameters to the Animator so animation state machine can react.
